...
public class ControladorAgregarSucursal implements ActionListener {
	...
	private ControladorAgregarSucursal(Sistema sistema, UsuarioDTO usuar) {
		...
		this.ventanaAgregarSucursal.getBtn_Cancelar().addActionListener(p -> this.ventanaAgregarSucursal.cerrar());
	...
	private void guardarSucursal(ActionEvent p) {
		...
		// validamos campos
		if (Validador.esNombreConNumerosEspaciosValido(S_nombreSucursal) && Validador.esNumeroSucursalValido(S_numero)
				&& Validador.esNombreConNumerosEspaciosValido(S_direccion)) {

			SucursalDTO nuevoSucursal = new SucursalDTO(0, S_nombreSucursal, S_direccion, Integer.parseInt(S_numero),
					"Activo");
